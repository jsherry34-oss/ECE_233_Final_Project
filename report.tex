...
\begin{abstract}
This report presents the implementation and evaluation of a single-shot beam training algorithm for wideband millimeter-wave (mmWave) OFDM systems using true-time-delay (TTD) arrays. Based on the work by Boljanovic et al.~\cite{boljanovic2020design}, I design frequency-dependent beam training codebooks for analog TTD arrays and implement a dictionary-based angle estimation algorithm. The system operates at 60~GHz with 2~GHz bandwidth and uses 128 base station antennas and 16 user equipment antennas. I evaluate the root mean square error (RMSE) of angle of arrival estimation under various signal-to-noise ratios and hardware impairment conditions, including gain, phase, and delay errors. Results demonstrate that the TTD-based approach enables efficient beam training with diversity factors of $R = 1, 2, 4$, achieving accurate angle estimation. However, I observe several discrepancies compared to the reference paper, including an elevated noise floor in my SNR results and a stronger sensitivity to phase errors relative to gain errors, suggesting potential implementation differences despite faithful adherence to the mathematical formulations in~\cite{boljanovic2020design}.
\end{abstract}

\section{Introduction}

Millimeter-wave (mmWave) communications are essential for achieving high data rates in fifth-generation and beyond wireless systems due to abundant available spectrum. However, mmWave systems face severe propagation loss and require beamforming with large antenna arrays at both the base station (BS) and user equipment (UE) to maintain reliable links~\cite{boljanovic2020design}.
...
True-time-delay (TTD) arrays offer a fundamentally different approach. Unlike phase shifters, TTD elements introduce frequency-dependent delays that enable different frequencies to probe different angular directions simultaneously~\cite{boljanovic2020design}. This frequency-diversity property allows a UE equipped with a TTD array to determine the dominant propagation direction using a single training symbol and simple frequency-domain digital signal processing.

In this project, I implement and evaluate the TTD-based beam training system proposed by Boljanovic et al.~\cite{boljanovic2020design}. My contributions include:
\begin{itemize}
    \item Design of frequency-dependent codebooks for analog TTD arrays with controllable diversity factor $R$
    \item Implementation of a dictionary-based super-resolution angle estimation algorithm
    \item Comprehensive evaluation of beam training performance under realistic hardware impairments
    \item Analysis of gain, phase, and delay errors on estimation accuracy
\end{itemize}
...
I consider a downlink beam training scenario between a BS and UE using orthogonal frequency division multiplexing (OFDM) with cyclic prefix. The system parameters follow the specifications in~\cite{boljanovic2020design} and are summarized in Table~\ref{tab:parameters}.
...
I implement a wideband geometric channel model with $L = 3$ multipath clusters, where the dominant path is 10~dB stronger than each of the other two paths. For a given sub-band $k$, the channel matrix is expressed as~\cite{boljanovic2020design}:
\begin{equation}
...
The BS transmits a power-normalized training pilot on $M$ selected subcarriers: $X[m] = M^{-1/2}$ for $m \in \mathcal{M}$. At the UE, the received signal at the $m$-th subcarrier is~\cite{boljanovic2020design}:
\begin{equation}
...
The key advantage of TTD arrays lies in their ability to synthesize frequency-dependent beams. The $n$-th element of the TTD weight vector at subcarrier $m$ is~\cite{boljanovic2020design}:
\begin{equation}
...
To introduce frequency diversity, we divide the bandwidth into $R$ distinct sub-bands. Within each sub-band, we associate $D$ uniformly spaced subcarriers with $D$ fixed directional beams. For a given direction $d$, the set of $R$ subcarriers mapped to that direction is~\cite{boljanovic2020design}:
\begin{equation}
...
To achieve the desired frequency-dependent beam pattern with $D = 32$ directions and diversity factor $R$, we design uniformly spaced delay and phase taps as~\cite{boljanovic2020design}:
\begin{align}
\tau_n &= (n-1)R/\text{BW} \\
...
The algorithm estimates the AoA by analyzing the received power across all $D$ directions. For direction $d$, the received signal power from the $R$ subcarriers in $\mathcal{M}_d$ provides independent samples of the channel. The maximum likelihood estimate of the power in direction $d$ is~\cite{boljanovic2020design}:
\begin{equation}
...
The expected power vector $\mathbf{p} = [p_1, \ldots, p_D]^T$ can be related to a dictionary matrix $\mathbf{B}$ via~\cite{boljanovic2020design}:
\begin{equation}
...
The vector $\mathbf{g}$ is sparse with significant energy only at the index corresponding to the dominant cluster's AoA. The AoA estimate is obtained by finding the dictionary column with highest correlation to the measured power vector~\cite{boljanovic2020design}:
\begin{equation}
\hat{\theta}^{(R)} = \xi_{q^*}, \quad \text{where} \quad q^* = \arg\max_q \frac{\hat{\mathbf{p}}^T [\mathbf{B}]_{:,q}}{\|[\mathbf{B}]_{:,q}\|}
\end{equation}

This super-resolution approach achieves accuracy beyond the angular spacing between training beams by jointly considering all direction measurements.

\section{Hardware Impairments}

Real TTD arrays suffer from hardware impairments that degrade beam training performance. I consider three types of errors as modeled in~\cite{boljanovic2020design}:
...
\end{equation}
where $\sigma_P$ is the standard deviation in radians or degrees.
...
For baseband (BB) TTD implementation, which is more robust to delay errors than radio frequency (RF) implementation, the impaired weight vector becomes~\cite{boljanovic2020design}:
\begin{equation}
...
All simulations were performed using MATLAB with $N_{\text{trials}} = 1000$ Monte Carlo trials. The signal-to-noise ratio is defined as $\text{SNR} = \sum_{l=1}^L \sigma_l^2 / \sigma_N^2$.

\subsection{Beam Patterns of TTD Codebook}

Figure~\ref{fig:beam_patterns} shows the beam patterns generated by the designed TTD codebook for $D = 32$ directions with diversity factor $R = 4$. The 32 beams uniformly cover the angular range from $-90°$ to $+90°$ with narrow mainlobes and low sidelobes, demonstrating effective spatial coverage for beam training. Each beam's frequency-dependent nature enables simultaneous probing of all directions using different subcarriers.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{results/plot1_beam_patterns.png}
\caption{Beam patterns of the designed TTD codebook showing 32 uniformly spaced directional beams. The $N_R = 16$ element array with TTD implementation creates narrow beams that cover the entire angular space.}
\label{fig:beam_patterns}
\end{figure}

\subsection{RMSE vs. SNR}

Figure~\ref{fig:rmse_snr} presents the root mean square error (RMSE) of angle estimation as a function of SNR for diversity factors $R = 1, 2, 4$ without hardware impairments. As expected, RMSE decreases with increasing SNR. The diversity factor significantly impacts performance: higher $R$ values provide better estimation accuracy by averaging over more independent channel realizations.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{results/plot2_rmse_vs_snr.png}
\caption{RMSE of angle estimation vs. SNR for diversity factors $R = 1, 2, 4$ without hardware impairments. Higher diversity factors achieve lower RMSE by exploiting frequency diversity, with $R = 4$ reaching sub-degree accuracy at high SNR.}
\label{fig:rmse_snr}
\end{figure}

At SNR = 0~dB, the RMSE is approximately 8° for $R = 1$, 4° for $R = 2$, and 2° for $R = 4$. At high SNR (20~dB), the RMSE approaches an error floor determined by the dictionary resolution $\pi/(2Q)$, achieving sub-degree accuracy. The performance improvement from $R = 1$ to $R = 4$ demonstrates the value of frequency diversity in frequency-selective channels.

\textbf{Implementation Note:} My results exhibit a higher noise floor compared to Figure 3 in~\cite{boljanovic2020design}, particularly noticeable at high SNR values where my RMSE does not decrease as steeply. Despite faithful implementation of all mathematical formulations from the reference paper, including the channel model, noise normalization, and estimation algorithm, my implementation appears to have an artificially elevated noise floor. This discrepancy may stem from subtle differences in simulation parameters, random number generation, or numerical precision that are not explicitly detailed in~\cite{boljanovic2020design}. The qualitative trends remain consistent with the reference, but absolute RMSE values are approximately 1-2 dB higher across all SNR ranges.
...
\caption{RMSE of angle estimation vs. gain error standard deviation $\sigma_A$ at SNR = 0~dB with $R = 4$. The algorithm maintains acceptable performance up to $\sigma_A \approx 2.5$~dB.}
\label{fig:rmse_gain}
\end{figure}

The results show that the algorithm is relatively robust to moderate gain errors. For $\sigma_A < 2$~dB, the RMSE remains below 5°. However, degradation occurs when $\sigma_A > 2.5$~dB, where the RMSE increases. This general trend is consistent with~\cite{boljanovic2020design}.

\textbf{Implementation Note:} My gain error results show a qualitatively similar trend to the red dashed line with stars in Figure 4 of~\cite{boljanovic2020design}, but the absolute RMSE values are elevated due to the aforementioned noise floor issue. More significantly, I observe that gain errors have less impact on my implementation than reported in the reference paper. In~\cite{boljanovic2020design}, gain errors cause more severe degradation than phase errors across all impairment levels, but my implementation shows the opposite behavior (see Section 6.4). This suggests potential differences in how hardware impairments interact with the estimation algorithm, possibly related to normalization procedures or the dictionary matching process.
...
\caption{RMSE of angle estimation vs. phase error standard deviation $\sigma_P$ at SNR = 0~dB with $R = 4$. Our implementation shows stronger sensitivity to phase errors compared to the reference paper.}
